...
io.on("connection", (socket) => {
  ...
  socket.on("initiate_call", async (data) => {
    ...
    io.to(relevantUser.socket_id).emit("receiving_call", {
      from: data.from,
      room: data.room,
    });
  });
...
mongoose.connection.once("open", () => {
  ...
  UsersChangeStream.on("change", async (change) => {
    if (
      ...
      !change.updateDescription.updatedFields.hasOwnProperty("socket_id")
    ) {
  ...
  const notificationsChangeStream = mongoose.connection
    .collection("notifications")
    .watch();
  notificationsChangeStream.on("change", async (change) => {
    if (
      change.operationType === "insert" &&
      change.fullDocument.title === "friend_request"
    ) {
      const relevantUser = await User.findById({
        _id: change.fullDocument.user_id,
      });
      io.to(relevantUser.socket_id).emit("notificationUpdate", change);
    }
    if (
      change.operationType === "insert" &&
      change.fullDocument.title === "connection_confirmation"
    ) {
      const relevantUser = await User.findById({
        _id: change.fullDocument.user_id,
      });
  ...
  chatsChangeStream.on("change", async (change) => {
    if (change.operationType === "update") {
      ...
      const relevantUsers = await User.find({
        friends: { $elemMatch: { chatId: updatedChat } },
      });
      io.to(relevantUsers[0].socket_id).emit("message_update", {
        updatedChat,
        messageInfo: {
          senderName: relevantUsers[1].username,
          senderId: relevantUsers[1]._id,
        },
      });
      io.to(relevantUsers[1].socket_id).emit("message_update", {
        updatedChat,
        messageInfo: {
          senderName: relevantUsers[0].username,
          senderId: relevantUsers[0]._id,
        },
      });
    }
  });
